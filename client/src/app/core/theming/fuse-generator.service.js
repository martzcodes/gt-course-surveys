(function () {
  ...
  function fuseGeneratorService($cookies, fuseTheming) {
    ...
    function generate() {
      const registeredThemes = fuseTheming.getRegisteredThemes();
      const registeredPalettes = fuseTheming.getRegisteredPalettes();

      angular.forEach(registeredThemes, (registeredTheme) => {
        themes[registeredTheme.name] = {};

        angular.forEach(registeredTheme.colors, (colorType, colorTypeName) => {
      ...
      angular.forEach(themes, (theme, themeName) => {
        styleVars = {};
        styleVars['@themeName'] = themeName;

        angular.forEach(theme, (colorTypes, colorTypeName) => {
          angular.forEach(colorTypes.levels, (colors, colorLevelName) => {
            angular.forEach(colors, (color, colorName) => {
    ...
    function processAndStoreThemes(_themes) {
      const themes = angular.copy(_themes);

      angular.forEach(themes, (theme) => {
        angular.forEach(theme, (colorType, colorTypeName) => {
